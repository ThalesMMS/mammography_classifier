...
class ImageViewerUI:
    ...
    def display_current_exam(self):
        ...
        # 1. Cria uma cópia editável da imagem.
        # 2. Converte de escala de cinza para BGR (formato de cor do OpenCV) para permitir texto colorido.
    ...
    def _draw_text_info(self, image, accession_number):
        ...
        if classification:
            ...
            # Cor do texto de status: Verde (formato BGR, então é (0, 255, 0))
    ...
    def show(self):
        ...
        while running:
            ...
            if key == ord('q') or key == 27: # 'q' ou ESC para sair
                running = False
            elif key == 82: # Seta para Cima (pode variar em outros sistemas)
                if self.data_manager.move_to_previous_folder():
                    running = self.display_current_exam()
            elif key == 84: # Seta para Baixo (pode variar em outros sistemas)
